...
use crate::routes::api::block::_block_id::course::create::CreateCourse;
...
pub(crate) async fn import_course(Path(block_id): Path<String>, Extension(state): Extension<Arc<ServerState>>,
                                  Extension(session): Extension<Arc<Session>>,
                                  Json(course_request): Json<ImportCourse>)
    -> Result<Json<Course>,AppError> {
    let con = &mut state.db_pool.get().unwrap();
    let original_course = course.find(&course_request.share_code).select(Course::as_select())
        .first(con).or_else(|_|AppError::resource_not_found().into())?;

    let original_components = CourseComponent::belonging_to(&original_course)
        .select(CourseComponent::as_select())
        .load(con).or_else(|_|AppError::unspecified_ise().into())?;
    ...
    insert_into(course).values(&new_course).execute(con).unwrap();
    insert_into(course_component).values(components).execute(con).unwrap();
    insert_into(course_subcomponent).values(subcomponents).execute(con).unwrap();
